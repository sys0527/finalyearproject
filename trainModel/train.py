...
input_shape = 100  # input shape for the model
...
def cnn_model(input_shape, num_classes):
    model = keras.Sequential([
        layers.Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(input_shape, 1)),
        layers.MaxPooling1D(pool_size=2),
        layers.Dense(50, activation='relu'),
        layers.Dropout(0.3),
        layers.Conv1D(filters=30, kernel_size=3, activation='relu', input_shape=(input_shape, 1)),
        layers.MaxPooling1D(pool_size=2),
...
my_model.save("gesture_model_virtual_mouse.h5")